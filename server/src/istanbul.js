// İBB Ulaşım Yönetim Merkezi kaza duyuruları (server/data/ibb/kazalar.json.gz) üzerinde filtre ve analizler.
...
export function loadIstanbul(file) {
...
export function computeHotspots(accidents, { limit = 10, cellSize = 0.005 } = {}) {
  ...
  for (const a of accidents) {
    ...
    c.lngSum += a.lng
    c.hours[a.hour]++
    const p = placeName(a.title)
    c.places.set(p, (c.places.get(p) ?? 0) + 1)
  }
