...
export default function WeekHourGrid({ grid }) {
  ...
  return (
    <div className="card chart span-3">
      <h2>Gün ve saate göre kaza yoğunluğu</h2>
      <p className="subtitle">Koyu hücre az, açık hücre çok kaza · Hücrenin üzerine gelince sayı görünür</p>
          ...
          <div role="row" className="whrow" key={d}>
            ...
            {row.map((v, h) => (
              <span
                ...
                title={`${DAYS[d]} ${String(h).padStart(2, '0')}:00–${String(h + 1).padStart(2, '0')}:00 · ${fmt(v)} kaza`}
                aria-label={`${DAYS[d]} saat ${h}: ${v} kaza`}
              />
            ))}
          </div>
        ))}
